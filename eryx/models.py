...
class Ensemble:
    ...
    def __init__(self, pdb_path, hsampling, ksampling, lsampling,
                 expand_p1=True, res_limit=0, batch_size=10000, n_processes=8):
        self.hsampling = hsampling
        self.ksampling = ksampling
        self.lsampling = lsampling
        self._setup(pdb_path, expand_p1, res_limit)
    ...
    def _setup(self, pdb_path, expand_p1, res_limit):
        """
    ...
    def apply_disorder(self, weights=None, num_state=-1, ensemble_dir=None):
        """
        ...
        asymmetric units. If an ensemble_dir is provided and num_state is 
        not -1, the unweighted results for the indicated state will be saved 
        to disk instead of the results from the full ensemble being computed.
        ...
        num_state : int
            index of the conformation / state in the PDB to save
        ensemble_dir : str
        ...
        # save unweighted SFs and apply weights during reconstruction
        if num_state!=-1:
            weights = np.ones((1, self.model.n_conf))      
        ...
        for asu in range(self.model.n_asu):
            ...
            for conf in range(self.model.n_conf):
                if (conf in [num_state]) or (num_state==-1):
                    ...
                    if ensemble_dir is not None:
                        np.save(os.path.join(ensemble_dir, f"conf{num_state:05}_asu{asu}.npy"), A)
